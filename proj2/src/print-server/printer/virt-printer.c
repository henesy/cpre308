...
int main(int argc, char* argv[])
{
	...
	// Defaults are good, segfaults are bad
	if(!hasname){
		printer_name_r = calloc(13 + strlen(default), sizeof(char));
		if(printer_name_r == NULL){
			perror("calloc");
			abort();
		}
		printer_name_w = calloc(13 + strlen(default), sizeof(char));
		if(printer_name_w == NULL){
			perror("calloc");
			abort();
		}
		strncat(printer_name_r, "./drivers/", 10);
		strncat(printer_name_r, default, strlen(default));
		strncat(printer_name_r, "-r", 2);
		
		strncat(printer_name_w, "./drivers/", 10);
		strncat(printer_name_w, default, strlen(default));
		strncat(printer_name_w, "-w", 2);
	}
